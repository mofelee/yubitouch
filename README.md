...
可选的 1Password SSH Agent fallback 只适用于同一把私钥先保存在 1Password、随后导入
YubiKey PIV 9A 的情况。先在 1Password **Settings > Developer** 中启用 SSH Agent，然后：
...
默认 socket 是 1Password for Mac 的
`~/Library/Group Containers/2BUA8C4S2C.com.1password/t/agent.sock`；只有使用非标准位置时才设置
`YUBITOUCH_FALLBACK_AGENT_SOCKET`。YubiTouch 会精确匹配现有 `public_key`，不会暴露或尝试
1Password 中的其他 key。处理签名时，YubiKey 已连接就不联系 fallback Agent；只有签名前
明确探测到 `not_detected` 才由 1Password 中的同一私钥签名，因此服务器、GitHub Signing Key 和 Git
`user.signingkey` 都不需要修改。

错误 PIN、PIN provider 取消、触摸取消/超时、设备探测失败和签名中途拔出不会 fallback。
一旦签名请求可能已发送给任一 backend，YubiTouch 不会改投另一个 backend。禁用 fallback：
...
1Password SSH Agent fallback 与上述 PIN provider 是独立路径。fallback 不读取 PIV PIN、
不加载 YKCS11，也不通过 Go SDK 解析 secret reference；它只连接配置的本地 SSH Agent
socket，并且只允许与 `public_key` 完全相同的 key。启用 fallback 表示安全边界从“必须持有
YubiKey”变为“持有 YubiKey，或者能够解锁保存同一私钥的 1Password”。
`status` 和 `doctor` 会执行无签名副作用的 identity 查询来报告 socket 可达性、目标 key 是否
存在和被过滤 key 的数量；这类诊断查询不读取 PIN，也不应触发 Touch ID。
...
| `fallback_agent_socket` | `YUBITOUCH_FALLBACK_AGENT_SOCKET` | 1Password for Mac SSH Agent socket |
...
| 1Password fallback 不可用 | 解锁桌面应用，启用 SSH Agent，运行 `doctor` 检查 socket 和目标 key。YubiTouch 不会尝试其他 key。 |
...
- 正常路径的私钥和签名操作留在 YubiKey/YKCS11/OpenSSH 中；显式启用时，同一私钥可由本地 1Password SSH Agent fallback 签名。
...
- 同一时间只有一个签名进入选定 backend；错误 PIN 不自动重试，也不触发 fallback。
- fallback 只在签名前明确确认 YubiKey `not_detected` 时选择；probe 失败、取消、超时和中途移除均 fail closed。
- fallback Agent 只允许与配置目标公钥完全相同的 key，其他 1Password identities 不向客户端暴露。
