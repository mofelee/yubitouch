...
启用回退后，daemon 只在连续两次设备探测都明确为 `not_detected`，且 1Password
socket 安全、可达、包含目标 key 并且没有其他 key 时，才把公共路径切到 1Password。
第一次缺卡探测、探测错误、回退检查失败、daemon 启停过程都失败闭合到 PIV，
不会在状态不明时暴露 1Password Agent。YubiKey 重新出现后，下一次成功探测就切回 PIV。
...
- 只有连续两次明确缺卡才进入回退；探测错误、身份不匹配、多余身份和 daemon 启停都失败闭合到 PIV。
...
和全屏 Space 已完成验收。#20 的 1Password 缺卡路由仍需按
[`docs/verification.md`](docs/verification.md) 记录真机结果，不应从自动化测试推断其已通过。
